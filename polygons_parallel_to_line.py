...
from PyQt4.QtGui import QIcon, QAction, QProgressBar
from qgis.core import (QgsMapLayerRegistry, QgsMapLayer, QgsSpatialIndex,
                       QgsField, QgsGeometry)
from qgis.utils import iface, QCoreApplication, QGis
...
class PolygonsParallelToLine:
    ...
    def result(self):
        ...
        linefeatures = {feature.id(): feature for (feature) in
                        linearLayer.getFeatures()}
        ...
        fieldName = "rotated"
        for i, attr in enumerate(polygonalLayer.pendingFields()):
            if attr.name() == fieldName:
                polygonalLayer.dataProvider().deleteAttributes([i])
                polygonalLayer.updateFields()
        polygonalLayer.dataProvider().addAttributes(
            [QgsField(fieldName, QVariant.Int)])
        polygonalLayer.updateFields()
        ...
        selectedOrNotState = self.dlg.selectedCheck.checkState()
        if selectedOrNotState == 2:
            pFeatures = polygonalLayer.selectedFeatures()
            self.progressBar = ShowProgress('PolygonsParallelToLine',
                                            'Обработка данных...',
                                            len(pFeatures))
        elif selectedOrNotState == 0:
            pFeatures = polygonalLayer.getFeatures()
            self.progressBar = ShowProgress('PolygonsParallelToLine',
                                       'Обработка данных...',
                                       polygonalLayer.featureCount())

        self.mainAction(pFeatures, index, linefeatures, distance, angle,
                        polygonalLayer)

        polygonalLayer.triggerRepaint()

    def mainAction(self, pFeatures, index, linefeatures, distance, angle,
                   polygonalLayer):
            ...
            if not polygon.geometry().isMultipart():
                ...
                if dist <= distance:
                    ...
                    for vertex in polygonVertexes:
                        ...
                        vertex_to_segment = vertex_geom.distance(
                            near_line.geometry())
                    ...
                    line1Azimuth = line1.asPolyline()[0].azimuth(
                        line1.asPolyline()[1])
                    line2Azimuth = line2.asPolyline()[0].azimuth(
                        line2.asPolyline()[1])

                    closestSegment = near_line.geometry().closestSegmentWithContext(
                        nearestVertex)
                    ...
                    def preRotation(segment, line):
                        ...
                        if (segment >= 0 and line >= 0) or (
                                        segment <= 0 and line <= 0):
                            delta = segment - line
                    ...
                    deltaAzimuth1 = preRotation(segmentAzimuth,
                                                line1Azimuth)
                    deltaAzimuth2 = preRotation(segmentAzimuth,
                                                line2Azimuth)
                    ...
                    if check == 2:
                        if delta1 <= angle and delta2 <= angle:
                            if line1.geometry().length() >= line2.geometry().length():
                                polygon.geometry().rotate(deltaAzimuth1,
                                                          centroid.asPoint())
                                rotationCheck = 1
                            elif line1.geometry().length() < line2.geometry().length():
                                polygon.geometry().rotate(deltaAzimuth2,
                                                          centroid.asPoint())
                                rotationCheck = 1
                            elif line1.geometry().length() == line2.geometry().length():
                                if delta1 > delta2:
                                    polygon.geometry().rotate(
                                        deltaAzimuth2, centroid.asPoint())
                                    rotationCheck = 1
                                elif delta1 < delta2:
                                    polygon.geometry().rotate(
                                        deltaAzimuth1, centroid.asPoint())
                                    rotationCheck = 1
                                elif delta1 == delta2:
                                    polygon.geometry().rotate(
                                        deltaAzimuth1, centroid.asPoint())
